...
public class SettingController {
    ...
    // 1. Mengambil data setting (Parameter email dibuat opsional/tidak wajib agar tidak memicu eror)
    @GetMapping("")
    public ResponseEntity<SettingModel> getSetting(@RequestParam String userId, @RequestParam(required = false) String email) {
    ...
    public ResponseEntity<?> changePassword(@RequestParam String userId, @RequestBody Map<String, String> request) {
        ...
        if (emailUser == null || emailUser.isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of("message", "Email user belum terdaftar di pengaturan profil"));
        }
