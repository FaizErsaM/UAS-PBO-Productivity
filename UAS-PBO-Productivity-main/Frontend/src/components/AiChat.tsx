...
const JiproLogoIcon = ({ className = "w-6 h-6", colorClass = "fill-white" }: { className?: string, colorClass?: string }) => (
  <svg 
    viewBox="0 0 100 100" 
...
export const AiChat = () => {
  ...
  const [messages, setMessages] = useState<{role: 'user' | 'ai', content: string}[]>([
    { role: 'ai', content: "Halo! Aku Jipro AI Problem Solver. Ada masalah belajar atau tugas yang ingin kamu pecahkan hari ini? Aku siap membantu menganalisis tugas, mengatur strategi habit, dan membantumu melakukan problem solving!" }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessages(prev => [...prev, { role: 'ai', content: "Sorry, I'm experiencing some difficulties right now. " + (data.error || "") }]);
      }
    } catch (err) {
  ...
  return (
    <>
      ...
      >
        <JiproLogoIcon className="w-8 h-8 group-hover:scale-110 transition-transform" colorClass="fill-white" />
      </button>
          ...
          >
            ...
            <div className="p-4 bg-gradient-to-r from-navy to-navy-light text-white flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-sm">JIPRO AI Problem Solver</h3>
                  <p className="text-[10px] text-purple-200 uppercase font-bold tracking-wider">Asisten Solusi Masalah</p>
                </div>
              </div>
              <button 
                ...
                <div key={idx} className={`flex gap-2 items-start ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  {msg.role === 'ai' && (
                    <div className="w-7 h-7 rounded-full bg-purple/10 border border-purple/20 flex items-center justify-center shrink-0 mt-0.5 shadow-sm">
                      <JiproLogoIcon className="w-[18px] h-[18px]" colorClass="fill-purple" />
                    </div>
                  )}
                  <div 
              ...
              {isTyping && (
                <div className="flex gap-2 items-start justify-start">
                  ...
                  <div className="bg-white border border-neutral-light rounded-2xl rounded-bl-sm p-3 shadow-sm flex gap-1.5 items-center">
                    <span className="w-1.5 h-1.5 bg-purple/60 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></span>
                    <span className="w-1.5 h-1.5 bg-purple/60 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></span>
                    <span className="w-1.5 h-1.5 bg-purple/60 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></span>
                  </div>
                </div>
              )}
