...
export const SettingsView = ({ onLogout }: { onLogout?: () => void }) => {
  ...
  const [profileGridItems, setProfileGridItems] = useState([
    { id: '1', label: 'Universitas', value: 'Universitas Indonesia', iconName: 'GraduationCap' },
    { id: '2', label: 'Angkatan / Tahun', value: '2024', iconName: 'Calendar' },
    { id: '3', label: 'Jurusan / Prodi', value: 'Teknik Informatika', iconName: 'Briefcase' },
  ...
  const renderGridIcon = (iconName: string) => {
    switch(iconName) {
      case 'GraduationCap': return <GraduationCap className="w-4 h-4 text-purple" />;
  ...
  const handleAddGridItem = (e: React.FormEvent) => {
    ...
    if (!gridLabel || !gridValue) {
      executeWithFeedback(async () => { throw new Error('Harap isi Label dan Nilai.'); }, '');
      return;
    }
    const newItem = {
    ...
    executeWithFeedback(async () => {}, 'Item grid profil baru berhasil ditambahkan!');
  };

  const handleStartEditGrid = (item: { id: string, label: string, value: string, iconName: string }) => {
  ...
  const handleSaveEditGrid = (e: React.FormEvent) => {
    ...
    setProfileGridItems(prev => prev.map(item => 
      item.id === editingGridId ? { ...item, label: gridLabel, value: gridValue, iconName: gridIconName } : item
    ));
    setEditingGridId(null);
    setGridLabel("");
    setGridValue("");
    executeWithFeedback(async () => {}, 'Item grid profil berhasil diperbarui!');
  };
  ...
  const handleTestBell = () => {
    try {
      ...
      audio.volume = 0.5;
      audio.play().then(() => {
        executeWithFeedback(async () => {}, 'Uji suara bel deadline berhasil dibunyikan!');
      }).catch(() => {
        executeWithFeedback(async () => {
          throw new Error('Gagal berinteraksi dengan audio browser. Silakan klik halaman lalu coba lagi.');
        }, '');
      });
    } catch(e) {
      executeWithFeedback(async () => {
  ...
  const handlePasswordChange = (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentPassword) {
      executeWithFeedback(async () => { throw new Error('Password saat ini harus diisi.'); }, '');
      return;
    }
    if (newPassword.length < 6) {
      executeWithFeedback(async () => { throw new Error('Password baru minimal harus 6 karakter.'); }, '');
      return;
    }
    if (newPassword !== confirmPassword) {
      executeWithFeedback(async () => { throw new Error('Konfirmasi password tidak cocok dengan password baru.'); }, '');
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <div className="flex flex-col items-center p-4">
              ...
              <h3 className="text-xl font-bold text-navy">{firstName} {lastName}</h3>
              ...
              <div className="w-full mt-6 border-t border-slate-100 pt-5">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center mb-3">Grid Data Profil</p>
                <div className="grid grid-cols-2 gap-2 text-left">
                  {profileGridItems.map(item => (
                    <div key={item.id} className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl flex flex-col justify-between">
                      <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1">
                        {renderGridIcon(item.iconName)}
                        {item.label}
                      </span>
                      <span className="text-xs font-semibold text-navy mt-1 truncate" title={item.value}>
                        {item.value}
                      </span>
                    </div>
                  ))}
        ...
        <div className="lg:col-span-2 space-y-6">
          {activeSection === 'profile' && (
            <div className="space-y-6">
              <Card>
                <h3 className="text-lg font-bold text-navy border-b border-slate-100 pb-4 mb-6">Informasi Pribadi</h3>
                
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Depan</label>
                      <input 
                        type="text" 
                        value={firstName} 
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nama Belakang</label>
                      <input 
                        type="text" 
                        value={lastName} 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Pekerjaan / Jabatan</label>
                    <input 
                      type="text" 
                      value={occupation} 
                  ...
                  <div className="mt-8 flex justify-end">
                    <button type="submit" className="px-6 py-2.5 bg-gradient-to-r from-purple to-soft-blue text-white font-bold rounded-xl hover:shadow-lg hover:shadow-purple/30 transition-all text-sm cursor-pointer">
                      Simpan Perubahan
                    </button>
                  </div>
                </form>
              ...
              <Card>
                <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                  <div>
                    <h3 className="text-lg font-bold text-navy">Kustomisasi Grid Informasi Profil</h3>
                    <p className="text-xs text-slate-500 mt-1">Sesuaikan grid informasi di kartu profil sisi kiri Anda.</p>
                  </div>
                ...
                {(isAddingGrid || editingGridId) && (
                  <form onSubmit={isAddingGrid ? handleAddGridItem : handleSaveEditGrid} className="mb-6 p-4 bg-slate-50 border border-slate-200/60 rounded-xl space-y-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold text-navy uppercase tracking-wider">
                        {isAddingGrid ? "Tambah Item Grid Baru" : "Edit Item Grid"}
                      </h4>
                      <button 
                        type="button" 
                        onClick={() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-1.5">Nama Label</label>
                        <input 
                          type="text" 
                          value={gridLabel} 
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-1.5">Nilai Data</label>
                        <input 
                          type="text" 
                          value={gridValue} 
                      ...
                      <div>
                        <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-widest mb-1.5">Pilih Ikon</label>
                        <select 
                          value={gridIconName} 
                        ...
                        >
                          <option value="GraduationCap">Akademik (Graduation)</option>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {profileGridItems.map(item => (
                    <div key={item.id} className="p-3.5 border border-slate-100 rounded-xl bg-slate-50/50 flex items-center justify-between group hover:border-slate-200 hover:bg-slate-50 transition-all">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-bold text-navy mt-0.5">{item.value}</p>
                        </div>
                      </div>
                  ...
                  {profileGridItems.length === 0 && (
                    <div className="col-span-2 text-center py-8 text-slate-400 text-xs font-semibold">
                      Belum ada kustom grid ditambahkan. Silakan klik "Tambah Blok" untuk membuat data grid profil Anda sendiri!
                    </div>
                  )}
                </div>
              </Card>
              ...
              <Card>
                <h3 className="text-lg font-bold text-navy border-b border-slate-100 pb-4 mb-6">Tampilan Aplikasi</h3>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1">Pilih tema utama aplikasi: Mode Terang (Light) atau Mode Gelap (Dark).</p>
                  </div>
          ...
          {activeSection === 'notifications' && (
            <div className="space-y-6">
              <Card>
                <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-6">
                  <h3 className="text-lg font-bold text-navy">Konfigurasi Pengingat & Suara</h3>
                  <button 
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between p-3 border border-slate-100 rounded-xl bg-slate-50">
                    <div className="max-w-[70%]">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Mainkan nada bel emas ketika deadline tugas tersisa dalam waktu kurang dari 1 hari secara real-time.
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <button 
                      ...
                      >
                        <div className={`w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm transition-transform ${soundEnabled ? 'right-1' : 'left-1'}`} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">Tampilkan banner notifikasi penting di dalam sistem dashboard.</p>
                    </div>
                    <div 
                      onClick={() => setPushEnabled(!pushEnabled)}
                      className={`w-11 h-6 rounded-full cursor-pointer relative transition-colors ${pushEnabled ? 'bg-purple' : 'bg-slate-300'}`}
                    >
                      <div className={`w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm transition-transform ${pushEnabled ? 'right-1' : 'left-1'}`} />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">Kirim ringkasan otomatis & pengingat deadline ke email yang terdaftar.</p>
                    </div>
                    <div 
                    ...
                    >
                      <div className={`w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm transition-transform ${emailDigestEnabled ? 'right-1' : 'left-1'}`} />
                    </div>
                  </div>
                  ...
                  {emailDigestEnabled && (
                    <div className="p-3 bg-purple/5 border border-purple/10 rounded-xl space-y-2">
                      <label className="block text-xs font-bold text-navy uppercase tracking-wider">Frekuensi Email Pengingat</label>
                      <select 
                      ...
                      >
                        <option value="immediate">Seketika (Saat deadline mendekati 1 hari)</option>
                        <option value="daily">Laporan Harian (Setiap pagi jam 08:00)</option>
                        <option value="weekly">Rangkuman Mingguan (Setiap hari senin)</option>
                      </select>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <div className="p-3 border border-emerald-100 bg-emerald-50/50 rounded-xl flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-emerald-900">Perlindungan Captcha Tulis Ulang Aktif</h4>
                    <p className="text-[11px] text-emerald-700 mt-0.5">
                      Sistem login telah dilindungi dengan Captcha Tulis Ulang Huruf Acak pada form Autentikasi. Ini mencegah serangan brute force dan bot liar ke akun privat Anda.
                    </p>
                  </div>
                </div>
              </Card>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4 mb-4 gap-2">
                  ...
                  {isEmailVerified ? (
                    <span className="px-2.5 py-1 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-lg flex items-center gap-1.5 self-start">
                      <ShieldCheck className="w-4 h-4 text-emerald-600 animate-pulse" /> Terverifikasi
                    </span>
                  ) : (
                    <span className="px-2.5 py-1 bg-amber-100 text-amber-700 text-xs font-bold rounded-lg flex items-center gap-1.5 self-start">
                      <ShieldAlert className="w-4 h-4 text-amber-600" /> Belum Terverifikasi
                    </span>
                  )}
                </div>
                ...
                {isEmailVerified ? (
                  <div className="space-y-4">
                    <div className="p-4 border border-emerald-100 bg-emerald-50/40 rounded-xl flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-sm font-bold text-emerald-950">Email Anda Terverifikasi</h4>
                        <p className="text-xs text-emerald-700 mt-1">
                          Email <strong className="font-semibold text-emerald-900">{email}</strong> telah diverifikasi secara sukses. Akun Anda sekarang memiliki kredensial penuh dan didukung proteksi keamanan ganda.
                        </p>
                      </div>
                    </div>
                    <div className="flex justify-end">
                      <button 
                        onClick={() => {
                          ...
                          setOtpSent(false);
                          setVerificationCode("");
                          executeWithFeedback(async () => {}, "Penyambungan email diset ulang.");
                        }}
                      ...
                      >
                        <RefreshCw className="w-3.5 h-3.5" /> Sambungkan Ulang Email
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Lakukan verifikasi email untuk mengonfirmasi bahwa Anda pemilik sah akun <strong className="text-navy">{email}</strong>. Ini penting demi kelancaran pemulihan kata sandi otomatis dan keamanan database.
                    </p>

                    {otpSent ? (
                      <form onSubmit={handleConfirmVerificationCode} className="space-y-3 p-4 bg-purple/5 border border-purple/10 rounded-xl">
                        <div className="flex items-center justify-between">
                          <label className="block text-xs font-bold text-navy uppercase tracking-wider">Masukkan Kode OTP</label>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="text-xs font-bold text-slate-700">Kirim kode verifikasi ke:</p>
                            <p className="text-sm font-semibold text-navy mt-0.5">{email}</p>
                          </div>
                        </div>
                        <button 
                          onClick={handleSendVerificationCode}
              ...
              <Card>
                <h3 className="text-lg font-bold text-navy border-b border-slate-100 pb-4 mb-6">Ubah Kata Sandi</h3>
                
                <form onSubmit={handlePasswordChange} className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kata Sandi Saat Ini</label>
                    <div className="relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Kata Sandi Baru</label>
                      <div className="relative">
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Konfirmasi Sandi Baru</label>
                      <div className="relative">
                        <Key className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-slate-400" />
                        <input 
                          type="password" 
                  ...
                  <div className="pt-2 flex justify-end">
                    <button type="submit" className="px-6 py-2.5 bg-purple text-white font-bold rounded-xl hover:shadow-lg hover:shadow-purple/30 transition-all text-sm cursor-pointer">
                      Perbarui Sandi
                    </button>
                  </div>
                </form>
              ...
              {/* Login Sessions */}
              <Card>
                <h3 className="text-base font-bold text-navy border-b border-slate-100 pb-3 mb-4">Sesi Login Terkini</h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <div className="flex items-center gap-2.5">
                      <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full" />
                      <div>
                        <p className="font-bold text-slate-800">Browser Aktif Saat Ini</p>
                        <p className="text-slate-500 text-[10px]">Cloud Run Sandbox Ingress • Jakarta, ID</p>
                      </div>
                    </div>
                    <span className="text-emerald-700 font-bold text-[10px] bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">Sesi Ini</span>
                  </div>
                </div>
              </Card>
            </div>
          ...
          {/* Danger Zone */}
          <Card>
            <h3 className="text-lg font-bold text-rose-600 border-b border-rose-100 pb-4 mb-6">Danger Zone (Keluar)</h3>
            <p className="text-xs text-slate-600 mb-4 font-medium">
              Keluar dari akun akan menghapus sesi login aktif saat ini. Anda harus masuk kembali dengan melengkapi Captcha untuk mengakses dashboard.
            </p>
            <button 
              onClick={onLogout}
