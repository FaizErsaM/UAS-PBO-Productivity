...
export const AnalyticsView = () => {
  ...
  const habitSuccessRates = habits.map((h) =>
    Math.min(100, (h.currentStreak / h.targetPeriod) * 100),
  );
  const avgHabitSuccess =
    habits.length > 0
  ...
  const habitProgressData = useMemo(() => {
    return habits.map((h) => ({
      name: h.habitName,
      progress: h.currentStreak,
      goal: h.targetPeriod,
      percentage: Math.min(
        100,
        Math.round((h.currentStreak / h.targetPeriod) * 100),
      ),
    }));
  }, [habits]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Your performance insights based on your current data.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <div className="mb-6 border-b border-slate-100 pb-3">
            <h3 className="text-lg font-semibold text-navy">
              Specific Tasks Completion
            </h3>
            <p className="text-sm text-slate-500 mt-1">
              Shows the completion status of each specific task you have
              inputted.
            </p>
          </div>
          {taskData.length > 0 ? (
        ...
        <Card>
          <div className="mb-6 border-b border-slate-100 pb-3">
            ...
            <p className="text-sm text-slate-500 mt-1">
              Tracks your cumulative daily progress against your target duration
              (e.g., 30 days) for each specific habit.
            </p>
          </div>
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value: unknown, _name: unknown, props: any) => [
                      `${value}% (${props.payload.currentStreak}/${props.payload.targetPeriod} days)`,
                      "Completion Rate",
                    ]}
                  />
                  <Legend wrapperStyle={{ fontSize: "12px" }} />
