...
export const Dashboard = ({
  ...
  const getHabitIcon = (title: string) => {
    if (title.includes("Work"))
      return <Flame className="w-4 h-4 md:w-5 md:h-5 text-orange-500" />;
    if (title.includes("Read"))
      return <BookOpen className="w-4 h-4 md:w-5 md:h-5 text-purple" />;
    if (title.includes("Water"))
      return <Droplets className="w-4 h-4 md:w-5 md:h-5 text-soft-blue" />;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="relative z-10 p-8 flex flex-col h-full justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold mb-2">
                {mainFocus ? mainFocus.title : "All Clear!"}
              </h2>
              {mainFocus && (
                <p className="text-slate-300 flex items-center gap-2">
                  <Clock className="w-4 h-4" /> Due:{" "}
                  {formatDeadline(mainFocus.deadline)}{" "}
                  {mainFocus.course && `• ${mainFocus.course}`}
                </p>
              )}
            </div>
        ...
        {/* Deep Work Modal */}
        {showDeepWork && (
          <DeepWorkView onClose={() => setShowDeepWork(false)} />
        )}
            ...
            <div className="relative pt-1">
              ...
              <div className="overflow-hidden h-2.5 mb-4 text-xs flex rounded-full bg-purple/10">
                <div
                  ...
                  className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-purple rounded-full transition-all duration-1000"
                ></div>
              </div>
            </div>
          </div>
        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              ...
              className="text-sm font-medium text-purple hover:text-purple-light transition-colors flex items-center"
            >
          ...
          <div className="space-y-3">
            {priorityTasks.length > 0 ? (
              priorityTasks.map((task) => {
                ...
                const isLocked =
                  hasUrgentTasks && !task.completed && priority !== "high";
                    ...
                    className={`group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border transition-all duration-200 ${
                      task.completed
                        ...
                        : isLocked
                          ...
                          : "bg-white border-neutral-light hover:border-purple/30 hover:shadow-sm"
                    }`}
                  >
                    <div className="flex items-start gap-4">
                      <button
                        ...
                        title={
                          isLocked ? "Complete high priority tasks first" : ""
                        }
                      >
                        {task.completed ? (
                      ...
                      <div>
                        <h4
                          className={`font-medium transition-colors ${task.completed ? "text-slate-400 line-through" : isLocked ? "text-slate-400" : "text-navy"}`}
                        >
                          {task.title}
                        </h4>
                    ...
                    {!task.completed && (
                      <div className="hidden sm:block">
                        <span
                          className={`text-xs font-semibold px-2.5 py-1 rounded-md border ${getPriorityColor(priority)} uppercase tracking-wider`}
                        >
        ...
        <Card>
          <div className="flex flex-col h-full">
                ...
                return (
                  <div key={habit.id} className="group">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      >
                        <div
                          className={`p-2 shrink-0 rounded-lg transition-colors mt-0.5 ${habit.lastCompletedDate === new Date().toISOString().split("T")[0] ? "bg-emerald-100 text-emerald-600" : "bg-slate-50 group-hover:bg-purple/5"}`}
                        >
                          {habit.lastCompletedDate ===
                          new Date().toISOString().split("T")[0] ? (
                            <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5" />
                          ) : (
                            getHabitIcon(habit.habitName)
                          )}
                        </div>
                        <span
                          className={`font-medium text-sm leading-normal mt-1 break-words max-w-full ${habit.lastCompletedDate === new Date().toISOString().split("T")[0] ? "text-emerald-700" : "text-navy"}`}
                        >
                          {habit.habitName}
                        </span>
                      </button>
            ...
            <button
              ...
              className="w-full mt-6 py-2.5 border-2 border-dashed border-slate-200 rounded-xl text-sm font-semibold text-slate-500 hover:text-navy hover:border-slate-300 hover:bg-slate-50 transition-all cursor-pointer"
            >
              + New Habit
            </button>
          </div>
